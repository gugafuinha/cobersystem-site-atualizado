...
const MODELOS = [
  {
    ...
    texto:
      'Sistema retrátil com telhas em alumínio — detalhes, galeria e especificações.',
  },
  {
...
export async function generateMetadata({
  ...
  const seo = getConteudoBairroLinha(slug, 'retratil');
      ...
      images: [
        {
          ...
          width: 1200,
          height: 900,
          alt: `Cobertura retrátil — ${bairro.nome}, São Paulo`,
        },
      ],
    },
    twitter: {
...
export default async function RetratilSaoPauloBairroPage({
  params,
}: {
  params: Promise<{ bairro: string }>;
}) {
  ...
  return (
    <SaoPauloBairroProdutoLayout
      ...
      hubLabel="Cobertura Retrátil"
      h1={`Cobertura Retrátil ${bairro.nome}`}
      seo={seo}
      imageSrc="/images/produtos/cobertura-retratil/aluminio/IMG_0305.jpg"
      imageWidth={1200}
      imageHeight={900}
      modelosHeading="Modelos da linha"
      modelosIntro={`Conheça as variantes retráteis para o seu projeto no ${bairro.nome}.`}
      modelosSection={modelosSection}
      vejaTambem="retratil"
      theme="blue"
      ctaTitle={`Orçamento no ${bairro.nome}`}
      ctaDescription="Entre em contato e solicite um orçamento personalizado para cobertura retrátil."
    />
  );
}
