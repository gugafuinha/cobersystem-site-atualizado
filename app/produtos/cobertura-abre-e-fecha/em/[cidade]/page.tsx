...
} from '@/lib/cobertura-abre-e-fecha-cidades';
...
export async function generateStaticParams() {
  return getSlugsCidadesAbreEFecha().map((cidade) => ({ cidade }));
}
...
export async function generateMetadata({
  ...
  const cidade = getCidadeAbreEFecha(cidadeParam);

  if (!cidade) {
    return { title: 'Página não encontrada' };
  }

  const path = `/produtos/cobertura-abre-e-fecha/em/${cidade.slug}`;
  const url = `${BASE}${path}`;
  const title = `Cobertura Abre e Fecha em ${cidade.nome} | Cobersystem`;
      ...
      images: [
        {
          ...
          width: 1200,
          height: 900,
          alt: `Cobertura abre e fecha em ${cidade.nome}`,
        },
      ],
    },
    twitter: {
...
export default async function CoberturaAbreEFechaEmCidadePage({
  params,
}: {
  params: Promise<{ cidade: string }>;
}) {
      ...
      <div className="container mx-auto max-w-6xl px-4">
          ...
          <div className="grid gap-8 md:grid-cols-2 md:items-start">
            <div className="relative aspect-[4/3] w-full overflow-hidden rounded-xl bg-gray-100">
              <OptimizedImage
                ...
                alt={`Cobertura abre e fecha em ${cidade.nome} - Cobersystem`}
                title={`Cobertura abre e fecha - ${cidade.nome}`}
                width={1200}
                height={900}
            ...
            <div>
              <h1 className="mb-4 text-4xl font-bold text-gray-800 md:text-5xl">
                Cobertura Abre e Fecha em {cidade.nome}
              </h1>
        ...
        {cidade.slug === 'sao-paulo' && (
          <section className="mb-12 rounded-lg border border-gray-200 bg-gray-50 p-6 text-center">
            ...
            <p className="mb-4 text-gray-600">
              Cobertura abre e fecha na capital, por região:
            </p>
            <ul className="flex flex-wrap justify-center gap-3">
              {SAO_PAULO_BAIRROS.map((b) => (
                <li key={b.slug}>
                  <Link
                    href={`/produtos/cobertura-abre-e-fecha/em/sao-paulo/${b.slug}`}
        ...
        <section className="mb-12">
          ...
          <p className="mb-6 text-lg text-gray-600">
            Conheça as variantes de cobertura abre e fecha para o seu projeto em{' '}
            {cidade.nome}.
          </p>
